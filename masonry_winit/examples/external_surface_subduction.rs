...
use masonry::app::{VisualLayer, VisualLayerPlan};
use masonry::core::{
...
use masonry_winit::app::{
    AppDriver, DriverCtx, LayerTextureTarget, NewWindow, PresentationTarget,
    SubductionPresenter, WindowId,
};
...
impl AppDriver for Driver {
    ...
    fn present_visual_layers(
        ...
        match self
            .compositor
            .present(target, layers, true, |target, layer| {
                self.demo_renderer.render_external_layer(target, layer)
            })
        {
            Ok(request_redraw) => Some(request_redraw),
            Err(err) => {
...
impl DemoRenderer {
    fn render_external_layer(
        ...
        self.render(
            ...
            target.format,
            target.output_size,
            layer.window_bounds().scale_from_origin(target.scale_factor),
            layer
                .window_clip_bounds()
                .map(|rect| rect.scale_from_origin(target.scale_factor)),
        );
        Ok(())
    }
}
