...
use crate::event_loop_runner::{Window, WindowState};
...
impl DriverCtx<'_, '_> {
    ...
    pub fn window_handle(&self, window_id: WindowId) -> &WindowHandle {
        let WindowState::Rendering { handle: window, .. } = &self.state.window(window_id).state
        else {
            panic!("window with id {window_id:?} is not in rendering state");
        };
        window
    }
