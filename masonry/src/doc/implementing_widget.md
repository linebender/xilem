...
Note that we don't store a size or a position: our widget's size is hinted at by its properties and ultimately decided by its parent, while its position is always managed by its parent.
...
We have nothing to do in `layout` because we have no children and no complex layout.
...
- Inside a `Widget` method. Most methods (`on_pointer_event`, `update`, `measure`, `layout`, etc) take a `&mut self` argument.
...
Let's define a setter for `ColorRectangle`:
...
By making `ColorRectangle`'s field private, and making it so the only way to mutate them is through a `WidgetMut`, we make it "watertight".
