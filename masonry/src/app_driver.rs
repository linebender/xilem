...
pub trait AppDriver {
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, window_id: WindowId, event: &WinitWindowEvent, masonry_state: &mut MasonryState<'_>) -> bool {
    ...
    fn device_event(&mut self, event_loop: &ActiveEventLoop, device_id: winit::event::DeviceId, event: &winit::event::DeviceEvent, masonry_state: &mut MasonryState<'_>) -> bool {
    ...
    fn user_event(&mut self, event_loop: &ActiveEventLoop, event: &accesskit_winit::Event, masonry_state: &mut MasonryState<'_>) -> bool {
        let _ = (event_loop, event, masonry_state);
        false
    }

    /// This corresponds to the winit::application::ApplicationHandler::new_events method.
    fn new_events(&mut self, event_loop: &ActiveEventLoop, cause: winit::event::StartCause, masonry_state: &mut MasonryState<'_>) {
    ...
    fn memory_warning(&mut self, event_loop: &ActiveEventLoop, masonry_state: &mut MasonryState<'_>) {
        let _ = (event_loop, masonry_state);
    }

    /// This corresponds to the winit::application::ApplicationHandler::about_to_wait method.   
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop, masonry_state: &mut MasonryState<'_>) {
        let _ = (event_loop, masonry_state);
    }
}
