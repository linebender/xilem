...
impl<const EDITABLE: bool> Widget for TextArea<EDITABLE> {
    // TODO: split implementation so that Action = () for TextArea<false>
    ...
    fn on_text_event(
        ...
        match event {
            TextEvent::Keyboard(key_event) => {
                ...
                match &key_event.key {
                    ...
                    Key::Named(NamedKey::Enter) => {
                        ...
                        if insert_newline {
                            ...
                            emit(TextAction::Entered(self.text().to_string()));
                        }
                    }
                ...
                if new_generation != self.rendered_generation {
                    if edited {
                        emit(TextAction::Changed(self.text().into_iter().collect()));
                        ctx.request_layout();
                    } else {
                        ctx.request_render();
                        ctx.set_ime_area(self.ime_area());
...
mod tests {
    ...
    fn insert_newline_behavior() {
        ...
        for scenario in scenarios {
            ...
            let (action, widget_id) = harness.pop_action_for::<TextArea<true>>().unwrap();
