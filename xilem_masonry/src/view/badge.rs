...
///
/// Uses Masonry's default overflow formatting (`99+`).
pub fn badge_count(count: u32) -> BadgeCount {
    BadgeCount {
        count,
        overflow: widgets::BadgeCountOverflow::default(),
    }
}

/// A badge displaying a numeric count with explicit overflow formatting.
pub fn badge_count_with_overflow(count: u32, overflow: widgets::BadgeCountOverflow) -> BadgeCount {
...
pub fn badge_count_nonzero(count: u32) -> Option<BadgeCount> {
    (count != 0).then(|| badge_count(count))
}
...
/// The [`View`] created by [`badge_count`] / [`badge_count_with_overflow`].
#[must_use = "View values do nothing unless provided to Xilem."]
