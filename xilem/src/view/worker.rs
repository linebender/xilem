    ...
    M: AnyMessage + 'static,
{
    ...
    fn build(&self, ctx: &mut ViewCtx) -> (Self::Element, Self::ViewState) {
        ...
        proxy
            .send_message(path.clone(), DynMessage::new(tx))
            .unwrap();
        let handle = ctx
            .runtime()
    ...
    fn message(
        ...
        if message.is::<UnboundedSender<V>>() {
            let sender = message.downcast::<UnboundedSender<V>>().unwrap();
            (self.store_sender)(app_state, *sender);
