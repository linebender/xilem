    ...
    Xilem<
        ...
        Box<
            dyn FnMut(
                ...
                WindowId,
                WindowAttrs<(bool, State)>,
                Box<AnyWidgetView<(bool, State)>>,
            )>,
        >,
    >
{
        ...
        State: 'static,
    {
        ...
        Xilem::new_inner(
            ...
            Box::new(move |(_, state)| {
                ...
                let on_close = move |(running, state): &mut (bool, State)| {
                    *running = false;
                ...
                std::iter::once((
                    ...
                    map_state(logic(state), |(_, state): &mut (bool, State)| state).boxed(),
                ))
            }),
            |(running, _)| *running,
        )
    ...
    WindowIter: Iterator<Item = (WindowId, WindowAttrs<State>, Box<AnyWidgetView<State>>)>,
{
    ...
    where
        State: AppState,
    {
        Self::new_inner(state, logic, State::keep_running)
    }

    fn new_inner(
        state: State,
        logic: Logic,
        keep_running: impl Fn(&State) -> bool + 'static,
    ) -> Self {
    ...
    pub fn into_driver(self, proxy: Arc<MasonryProxy>) -> MasonryDriver<State, Logic> {
        MasonryDriver::new(
            self.state,
            self.logic,
            self.fonts,
