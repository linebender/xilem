...
use masonry::widgets::ContentHost;
...
pub type AnyWidgetView<State, Action = ()> =
    dyn AnyView<State, Action, ViewCtx, Pod<ContentHost>> + Send + Sync;

impl<W: Widget + FromDynWidget + ?Sized> SuperElement<Pod<W>, ViewCtx> for Pod<ContentHost> {
    fn upcast(ctx: &mut ViewCtx, child: Pod<W>) -> Self {
        ctx.create_pod(ContentHost::new(child.new_widget.erased()))
    }
    ...
    fn with_downcast_val<R>(
        ...
        let ret = {
            let mut child = ContentHost::child_mut(&mut this);
...
impl<W: Widget + FromDynWidget + ?Sized> AnyElement<Pod<W>, ViewCtx> for Pod<ContentHost> {
    fn replace_inner(mut this: Self::Mut<'_>, child: Pod<W>) -> Self::Mut<'_> {
        ContentHost::replace_child(&mut this, child.new_widget.erased());
        this
    }
}
