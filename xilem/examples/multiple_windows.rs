...
use xilem::{
    AnyWidgetView, AppState, EventLoop, EventLoopBuilder, WidgetView, WindowAttrs, WindowId, Xilem,
};
...
fn app_logic(
    state: &mut State,
) -> impl Iterator<Item = (WindowId, WindowAttrs<State>, Box<AnyWidgetView<State>>)> + use<> {
    std::iter::once((
        state.main_window_id,
        WindowAttrs::new("Multiple windows").on_close(|state: &mut State| {
            state.running = false;
        }),
        flex((
            label(format!(
    ...
    .chain(
        state
            ...
            .map(|(window_id, Counter { name, value })| {
                ...
                (
                    window_id,
                    WindowAttrs::new(name).on_close(move |state: &mut State| {
