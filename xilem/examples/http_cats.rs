...
struct HttpCats {
    ...
    download_sender: UnboundedSender<u32>,
}
        ...
        flex((
            ...
            button("Select", move |state: &mut HttpCats| {
                ...
                if matches!(status.image, ImageState::NotRequested) {
                    state.download_sender.send(code).unwrap();
...
fn run(event_loop: EventLoopBuilder) -> Result<(), EventLoopError> {
    let data = HttpCats {
        ...
        selected_code: None,
        download_sender: unbounded_channel().0, // initialize with closed sender, will be overwritten once the view is built
    };
